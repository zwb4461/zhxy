...
export default {
  ...
  methods: {
    ...
    dataInit() {
      ...
      http
        .selectEleSchool({ schoolId: this.schoolId })
        .then(res => {
          let arr = JSON.parse(JSON.stringify(res.data.categoryList));
          let arr2 = JSON.parse(JSON.stringify(res.data.eleSchool));
          arr.forEach(element => {
            element.startTime = this.moment(element.startTime);
            element.endTime = element.endTime.split(",");
