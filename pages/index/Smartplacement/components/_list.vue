    ...
    <div class="box">
      ...
      <div class="card" v-for="(item, index) in this.list" :key="index">
        ...
        >
          ...
          <div class="content">
            <a-spin size="large" tip="数据更新中" :spinning="item.loading">
              已分班：{{ intoperpson[index] }}人
              <br />
              未分班：{{ noperson[index] }}人
              <br />
              班级数：{{ item.deptclass.length }}个
              <br />
            </a-spin>
          </div>
        </a-card>
...
export default {
  ...
  methods: {
    ...
    //跳转到该类别下的学生详情
    toInfo({ studenttype, id, deptclass, isClose, isconfirm }) {
      if (isClose == 1) {
        this.addPans({ studenttype, id, deptclass, isconfirm });
      } else {
